...
fn decode_bencoded_list<T: AsRef<[u8]>>(encoded_value: T) -> (usize, serde_json::Value) {
...
fn decode_bencoded_dict<T: AsRef<[u8]>>(encoded_value: T) -> (usize, serde_json::Value) {
    // Get string from start until 'e'
    let encoded_value = encoded_value.as_ref();
    let mut encoded_value = &encoded_value[1..];
    let mut ending_index = 1;
...
#[allow(dead_code)]
fn decode_bencoded_value<T: AsRef<[u8]> + std::fmt::Debug>(
    encoded_value: T,
